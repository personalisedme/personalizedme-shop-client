...
const ENVIRONMENT_IS_NODE = 'string' == typeof globalThis.process?.versions?.node,
  ENVIRONMENT_IS_DENO =
    ('object' == typeof window && 'Deno' in window) || ('object' == typeof self && 'Deno' in self);
  ...
  try {
    if (ENVIRONMENT_IS_NODE) {
      ...
      t.open('GET', e, !1),
        ...
        t.send(),
        200 === t.status
          ? globalThis.gdPicture.module.FS.writeFile(r, stringToArrayBuffer(t.response))
          : console.error(`Could not retrieve resource. Status: ${t.status}`);
    }
  } catch (e) {
    console.error(`Could not retrieve resource. Exception: ${e}`);
  }
}
